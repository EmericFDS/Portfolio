...
function initSpotlightAndCursor() {
    ...
    // Bolt Optimization: Avoid global window mousemove layout thrashing.
    // Instead of querying getBoundingClientRect() for every card on every mouse movement across the page,
    // attach mousemove listeners to each card and cache its bounding rect on mouseenter/resize.
    cards.forEach(card => {
        let rect = null;
