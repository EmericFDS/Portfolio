...
function initSpotlightAndCursor() {
    ...
    // Avoid layout thrashing from calling getBoundingClientRect() on all cards on every global mousemove.
    // Attach listeners directly to each card, caching bounding rect on hover and updating CSS custom properties
    // only for the active hovered card.
    cards.forEach(card => {
        let rect = null;
        ...
        card.addEventListener('mouseleave', invalidate, { passive: true });
